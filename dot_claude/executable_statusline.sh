...
#   🕐 ███░░░░░░░░░░░░ 23% │ 📅 █████████░░░░░░ 60%
#
# 加えて、他のスクリプト(自動リトライ等)が読めるように
# rate_limits (5時間/7日ウィンドウの使用率) を ~/.claude/rate_limit_status.json へ書き出す。
#
set -u

input=$(cat)

# --- rate_limitsをファイルに保存(監視スクリプト用) ---
STATUS_FILE="${CLAUDE_RATE_LIMIT_STATUS_FILE:-$HOME/.claude/rate_limit_status.json}"
echo "$input" | jq -c --arg ts "$(date +%s)" '{
