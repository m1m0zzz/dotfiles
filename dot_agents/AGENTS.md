...
- ターミナル環境の設定を変更する時:
  `~/.claude/my-contexts/user-terminal-setup.md`
- git commit する時:
  `~/.claude/my-contexts/git-commit-identity.md`
- ワークツリーを作る/消す時（**EnterWorktree で直接作らず `gwt` を使う**）:
  `~/.claude/my-contexts/worktree.md`
...
ユーザーが「マージしました」と伝えたら、リモートの最新状態を取得し、
対象リポジトリを最新の `main` に切り替える。
